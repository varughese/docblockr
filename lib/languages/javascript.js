...
JsParser.prototype.parse_function = function(line) {
    ...
    if('directive|service|factory|provider|value|constant|config'.indexOf(matches.name2) > -1) {
        name = '@' + matches.name2 + " "  + matches.args.replace(/\s+/, '').substring(matches.args.indexOf('\'')+1, matches.args.indexOf(',')-1);
        args = '';
    }
